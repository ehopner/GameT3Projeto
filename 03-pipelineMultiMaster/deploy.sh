...
ID_W4=$(terraform output | grep 'k8s-workers 4 -' | awk '{print $4;exit}')
ID_W4_DNS=$(terraform output | grep 'k8s-workers 4 -' | awk '{print $9;exit}' | cut -b 8-)
...
# ANSIBLE_OUT=$(ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i hosts provisionar.yml -u ubuntu --private-key "/var/lib/jenkins/id_rsa")
ANSIBLE_OUT=$(ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i hosts provisionar.yml -u ubuntu --private-key $TF_VAR_PATH_KEY)
...
ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i hosts 2-provisionar-k8s-master-auto-shell.yml -u ubuntu --private-key $TF_VAR_PATH_KEY
